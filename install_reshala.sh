...
# ============================================================ #
# ==      ИНСТРУМЕНТ «РЕШАЛА» v0.8 - FULL VERSION      ==
# ============================================================ #
# ==    Исправлена работа от root, перестроено меню.        ==
# ============================================================ #
...
run_update() {
    ...
    if [[ "$confirm_update" != "y" && "$confirm_update" != "Y" ]]; then echo -e "${C_YELLOW}🤷‍♂️ Ну и сиди со старьём. Твоё дело.${C_RESET}"; wait_for_enter; return; fi
    echo -e "${C_CYAN}🔄 Качаю свежак...${C_RESET}"; local TEMP_SCRIPT; TEMP_SCRIPT=$(mktemp)
    local url_with_buster="${SCRIPT_URL}?cache_buster=$(date +%s)$(shuf -i 1000-9999 -n 1)"
...
net.ipv4.tcp_wmem = 4096 65536 16777216" | run_cmd tee "$CONFIG_SYSCTL" > /dev/null; log "🔥 Применяю настройки..."; run_cmd sysctl -p "$CONFIG_SYSCTL" >/dev/null; echo ""; echo "--- КОНТРОЛЬНЫЙ ВЫСТРЕЛ ---"; echo "Новый алгоритм: $(sysctl -n net.ipv4.tcp_congestion_control)"; echo "Новый планировщик: $(sysctl -n net.core.default_qdisc)"; echo "---------------------------"; echo -e "${C_GREEN}✅ Твоя тачка теперь — ракета. (CC: $preferred_cc, QDisc: $preferred_qdisc)${C_RESET}";
}
...
disable_ipv6() { if [ ! -d "/proc/sys/net/ipv6" ]; then echo -e "❌ ${C_YELLOW}Тут нечего отключать. Провайдер уже всё отрезал за тебя.${C_RESET}"; return; fi; if [ "$(cat /proc/sys/net/ipv6/conf/all/disable_ipv6)" -eq 1 ]; then echo "⚠️ IPv6 уже кастрирован."; return; fi; echo "🔪 Кастрирую IPv6... Это не больно. Почти."; run_cmd tee /etc/sysctl.d/98-reshala-disable-ipv6.conf > /dev/null <<EOL
# === КОНФИГ ОТ РЕШАЛЫ: IPv6 ОТКЛЮЧЁН ===
net.ipv6.conf.all.disable_ipv6 = 1
net.ipv6.conf.default.disable_ipv6 = 1
EOL
    run_cmd sysctl -p /etc/sysctl.d/98-reshala-disable-ipv6.conf > /dev/null; log "-> IPv6 кастрирован через sysctl."; echo -e "${C_GREEN}✅ Готово. Теперь эта тачка ездит только на нормальном топливе.${C_RESET}"; }
enable_ipv6() { if [ ! -d "/proc/sys/net/ipv6" ]; then echo -e "❌ ${C_YELLOW}Тут нечего включать. Я не могу пришить то, что отрезано с корнем.${C_RESET}"; return; fi; if [ ! -f /etc/sysctl.d/98-reshala-disable-ipv6.conf ] && [ "$(cat /proc/sys/net/ipv6/conf/all/disable_ipv6)" -eq 0 ]; then echo "✅ IPv6 и так работает. Не мешай ему."; return; fi; echo "💉 Возвращаю всё как было... Реанимация IPv6."; run_cmd rm -f /etc/sysctl.d/98-reshala-disable-ipv6.conf; run_cmd tee /etc/sysctl.d/98-reshala-enable-ipv6.conf > /dev/null <<EOL
# === КОНФИГ ОТ РЕШАЛЫ: IPv6 ВКЛЮЧЁН ===
net.ipv6.conf.all.disable_ipv6 = 0
net.ipv6.conf.default.disable_ipv6 = 0
EOL
    run_cmd sysctl -p /etc/sysctl.d/98-reshala-enable-ipv6.conf > /dev/null; run_cmd rm -f /etc/sysctl.d/98-reshala-enable-ipv6.conf; log "-> IPv6 реанимирован."; echo -e "${C_GREEN}✅ РЕАНИМАЦИЯ ЗАВЕРШЕНА.${C_RESET}"; }
ipv6_menu() { while true; do clear; echo "--- УПРАВЛЕНИЕ IPv6 ---"; echo -e "Статус IPv6: $(check_ipv6_status)"; echo "--------------------------"; echo "   1. Кастрировать (Отключить)"; echo "   2. Реанимировать (Включить)"; echo "   b. Назад в главное меню"; read -r -p "Твой выбор: " choice; case $choice in 1) disable_ipv6; wait_for_enter;; 2) enable_ipv6; wait_for_enter;; [bB]) break;; *) echo "1, 2 или 'b'. Не тупи."; sleep 2;; esac; done; }
view_logs_realtime() { local log_path="$1"; local log_name="$2"; if [ ! -f "$log_path" ]; then echo -e "❌ ${C_RED}Лог '$log_name' пуст.${C_RESET}"; sleep 2; return; fi; echo "[*] Смотрю журнал '$log_name'... (CTRL+C, чтобы свалить)"; local original_int_handler=$(trap -p INT); trap "echo -e '\n${C_GREEN}✅ Возвращаю в меню...${C_RESET}'; sleep 1;" INT; (run_cmd tail -f -n 50 "$log_path" | awk -F ' - ' -v C_YELLOW="$C_YELLOW" -v C_RESET="$C_RESET" '{print C_YELLOW $1 C_RESET "  " $2}') || true; if [ -n "$original_int_handler" ]; then eval "$original_int_handler"; else trap - INT; fi; return 0; }
view_docker_logs() { local service_path="$1"; local service_name="$2"; if [ -z "$service_path" ] || [ ! -f "$service_path" ]; then echo -e "❌ ${C_RED}Путь — хуйня.${C_RESET}"; sleep 2; return; fi; echo "[*] Смотрю потроха '$service_name'... (CTRL+C, чтобы свалить)"; local original_int_handler=$(trap -p INT); trap "echo -e '\n${C_GREEN}✅ Возвращаю в меню...${C_RESET}'; sleep 1;" INT; (cd "$(dirname "$service_path")" && run_cmd docker compose logs -f) || true; if [ -n "$original_int_handler" ]; then eval "$original_int_handler"; else trap - INT; fi; return 0; }
uninstall_script() { echo -e "${C_RED}Точно хочешь выгнать Решалу?${C_RESET}"; read -p "Это снесёт скрипт, конфиги и алиасы. (y/n): " confirm; if [[ "$confirm" != "y" && "$confirm" != "Y" ]]; then echo "Правильное решение."; wait_for_enter; return; fi; echo "Прощай, босс. Начинаю самоликвидацию..."; if [ -f "$INSTALL_PATH" ]; then run_cmd rm -f "$INSTALL_PATH"; echo "✅ Главный файл снесён."; log "-> Скрипт удалён."; fi; if [ -f "/root/.bashrc" ]; then run_cmd sed -i "/alias reshala='sudo reshala'/d" /root/.bashrc; echo "✅ Алиас выпилен."; log "-> Алиас удалён."; fi; if [ -f "$CONFIG_FILE" ]; then rm -f "$CONFIG_FILE"; echo "✅ Конфиг стёрт."; log "-> Конфиг удалён."; fi; if [ -f "$LOGFILE" ]; then run_cmd rm -f "$LOGFILE"; echo "✅ Журнал сожжён."; fi; echo -e "${C_GREEN}✅ Самоликвидация завершена.${C_RESET}"; echo "   Переподключись, чтобы алиас 'reshala' сдох."; exit 0; }

# --- МОДУЛЬ БЕЗОПАСНОСТИ ---
ssh_key_manager() {
    clear; echo -e "${C_CYAN}--- МАССОВОЕ ДОБАВЛЕНИЕ SSH-КЛЮЧЕЙ ---${C_RESET}"; echo -e "Этот модуль поможет тебе закинуть твой SSH-ключ на все твои серверы."; echo -e "\n${C_BOLD}[ ШАГ 1: Подготовь публичный ключ ]${C_RESET}"; echo "На ${C_YELLOW}ТВОЁМ ЛИЧНОМ КОМПЬЮТЕРЕ${C_RESET} (не на этом сервере), открой терминал и выполни:"; echo -e "  1. Если ключа нет, создай его: ${C_GREEN}ssh-keygen -t ed25519${C_RESET}"; echo -e "  2. Посмотри и скопируй свой ${C_YELLOW}ПУБЛИЧНЫЙ${C_RESET} ключ: ${C_GREEN}cat ~/.ssh/id_ed25519.pub${C_RESET}"; echo "     Скопируй всю строку, которая начинается с 'ssh-ed25519...'"; read -p $'\nТы скопировал свой ПУБЛИЧНЫЙ ключ и готов продолжить? (y/n): ' confirm_key; if [[ "$confirm_key" != "y" && "$confirm_key" != "Y" ]]; then echo -e "${C_RED}Отмена. Возвращаю в меню.${C_RESET}"; sleep 2; return; fi; echo -e "\n${C_BOLD}[ ШАГ 2: Вставь свой ключ ]${C_RESET}"; read -p "Вставь сюда свой публичный ключ (ssh-ed25519...): " PUBKEY; if ! [[ "$PUBKEY" =~ ^ssh-(rsa|dss|ed25519|ecdsa) ]]; then echo -e "${C_RED}❌ Это не похоже на SSH-ключ. Давай по новой.${C_RESET}"; wait_for_enter; return; fi; local SERVERS_FILE="servers.txt"; echo -e "\n${C_BOLD}[ ШАГ 3: Подготовь список серверов ]${C_RESET}"; echo "Теперь на этом сервере, в текущей директории, создай файл с именем ${C_YELLOW}${SERVERS_FILE}${C_RESET}"; echo "Внутри файла укажи каждый сервер на новой строке в формате ${C_GREEN}пользователь@адрес${C_RESET}."; echo "Пример: root@1.2.3.4 или admin@mydomain.com"; read -p $'\nТы создал файл servers.txt и готов к установке? (y/n): ' confirm_servers; if [[ "$confirm_servers" != "y" && "$confirm_servers" != "Y" ]]; then echo -e "${C_RED}Отмена. Возвращаю в меню.${C_RESET}"; sleep 2; return; fi; if [ ! -f "$SERVERS_FILE" ]; then echo -e "${C_RED}❌ Файл '${SERVERS_FILE}' не найден. Убедись, что он в той же папке, откуда запущен Решала.${C_RESET}"; wait_for_enter; return; fi; echo -e "\n${C_BOLD}[ ШАГ 4: Установка ключа на серверы ]${C_RESET}"; echo "Сейчас я буду по очереди подключаться к каждому серверу."; echo -e "${C_YELLOW}Тебе нужно будет ввести пароль для каждого сервера.${C_RESET}"; wait_for_enter; if command -v ssh-copy-id &> /dev/null; then
        echo "Использую 'ssh-copy-id' (безопасный метод)..."; local TEMP_KEY_FILE; TEMP_KEY_FILE=$(mktemp); echo "$PUBKEY" > "$TEMP_KEY_FILE"
        while IFS= read -r host || [[ -n "$host" ]]; do if [[ -z "$host" ]] || [[ "$host" =~ ^# ]]; then continue; fi; echo -e "\n${C_CYAN}--> Добавляю ключ на $host...${C_RESET}"; if ssh-copy-id -i "$TEMP_KEY_FILE" -o ConnectTimeout=10 -o StrictHostKeyChecking=no "$host"; then echo -e "    ${C_GREEN}✅ Успех!${C_RESET}"; else echo -e "    ${C_RED}❌ Ошибка. Проверь пароль или доступность хоста.${C_RESET}"; fi; done < "$SERVERS_FILE"; rm "$TEMP_KEY_FILE"
    else
        echo "ВНИМАНИЕ: 'ssh-copy-id' не найден. Использую ручной метод (менее надёжный)."; while IFS= read -r host || [[ -n "$host" ]]; do if [[ -z "$host" ]] || [[ "$host" =~ ^# ]]; then continue; fi; echo -e "\n${C_CYAN}--> Добавляю ключ на $host...${C_RESET}"; local ssh_command="mkdir -p ~/.ssh && touch ~/.ssh/authorized_keys && grep -q -F '$PUBKEY' ~/.ssh/authorized_keys || echo '$PUBKEY' >> ~/.ssh/authorized_keys && chmod 700 ~/.ssh && chmod 600 ~/.ssh/authorized_keys"; if ssh -o ConnectTimeout=10 -o StrictHostKeyChecking=no "$host" "$ssh_command"; then echo -e "    ${C_GREEN}✅ Успех!${C_RESET}"; else echo -e "    ${C_RED}❌ Ошибка. Проверь пароль или доступность хоста.${C_RESET}"; fi; done < "$SERVERS_FILE"
...
show_menu() {
    while true; do
        ...
        echo -e "   [6] Безопасность сервера ${C_YELLOW}(На начальной стадии)${C_RESET}"
