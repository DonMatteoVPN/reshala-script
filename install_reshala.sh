...
# ============================================================ #
# ==      ИНСТРУМЕНТ «РЕШАЛА» v0.3551 dev - FINAL CUT         ==
# ============================================================ #
...
# --- МОДУЛЬ ОБНОВЛЕНИЯ (С ЧИСТЫМИ ЛОГАМИ) ---
check_for_updates() {
    UPDATE_AVAILABLE=0
    LATEST_VERSION=""
...
if [[ "${1:-}" == "install" ]]; then
    ...
    # Убираем ловушку, чтобы не мешала при выходе из меню логов
    trap - INT TERM EXIT
    show_menu
fi