...
# ============================================================ #
# ==      ИНСТРУМЕНТ «РЕШАЛА» v2.2111 - BACK TO BASICS      ==
# ============================================================ #
        ...
        case $choice in
            ...
            2)
                ...
                if [[ "$confirm" == "y" || "$confirm" == "Y" ]]; then
                    ...
                    echo ""; echo "${C_GREEN}✅ Простая очистка завершена.${C_RESET}"
                fi
                wait_for_enter
                ;;
            3)
                echo ""; echo "💥 Полная очистка образов (image prune -a)"
                echo "Удаляет ВСЕ образы, не используемые запущенными контейнерами."
                echo "${C_RED}Внимание:${C_RESET} Если у тебя есть остановленные контейнеры, их образы тоже удалятся!"
                read -p "Точно продолжить? (y/n): " confirm
                if [[ "$confirm" == "y" || "$confirm" == "Y" ]]; then
                    docker image prune -a -f
                    echo ""; echo "${C_GREEN}✅ Полная очистка образов завершена.${C_RESET}"
                fi
                wait_for_enter
                ;;
            4)
                echo ""; echo "🗑️ Очистка томов (volume prune)"
                echo "${C_RED}ОСТОРОЖНО!${C_RESET} Удаляет все тома, не используемые ни одним контейнером (даже остановленным)."
                echo "Если у тебя есть важные данные в остановленных контейнерах — НЕ ДЕЛАЙ ЭТОГО!"
                read -p "Ты уверен, что хочешь это сделать? (y/n): " confirm
                if [[ "$confirm" == "y" || "$confirm" == "Y" ]]; then
                    docker volume prune -f
                    echo ""; echo "${C_GREEN}✅ Очистка томов завершена.${C_RESET}"
                fi
                wait_for_enter
                ;;
            5)
                ...
                ;;
            [bB]) break ;;
            *) echo "${C_RED}Не та кнопка. Выбирай 1-5 или 'b'.${C_RESET}"; sleep 2 ;;
        esac
    done
