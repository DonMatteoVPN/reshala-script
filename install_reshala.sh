...
# ============================================================ #
# ==      ИНСТРУМЕНТ «РЕШАЛА» v0.4 dev - FINAL CUT         ==
# ============================================================ #
# ==    Возвращена система обновлений из v0.3451.            ==
# ==    Изменено расположение уведомления об обновлении.     ==
# ============================================================ #

set -euo pipefail

# --- КОНСТАНТЫ И ПЕРЕМЕННЫЕ ---
readonly VERSION="v0.4 dev"
...
# --- МОДУЛЬ ОБНОВЛЕНИЯ (ВОЗВРАЩЕННЫЙ ИЗ v0.3451) ---
check_for_updates() {
    UPDATE_AVAILABLE=0
    LATEST_VERSION=""
...
show_menu() {
    while true; do
        ...
        if [[ ${UPDATE_AVAILABLE:-0} -eq 1 ]]; then 
            ...
            echo -e "${C_RED}⚠️ Ошибка проверки обновлений (см. лог)${C_RESET}\n"
        fi

        echo "Чё делать будем, босс?"; echo ""
