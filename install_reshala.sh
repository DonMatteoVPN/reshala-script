#!/bin/bash
# ============================================================ #
# ==      ИНСТРУМЕНТ «РЕШАЛА» v2.21132 - Пока хуячим   ==
# ============================================================ #
set -uo pipefail
...
show_menu() {
    # === ЗАЩИТА ОТ CTRL+C ===
    # Ставим ловушку: при попытке прерывания (SIGINT) просто пишем сообщение
    trap 'printf "\n%b\n" "${C_RED}🛑 Куда собрался? Жми [q], чтобы выйти как нормальный пацан.${C_RESET}"' SIGINT
    ...
    while true; do
        ...
        # Мы её ловим, сбрасываем и идем на новый круг цикла.
        if ! read -r -p "Твой выбор, босс: " choice; then
            echo "" # Просто перевод строки после ^C
            continue
        fi
        ...
        case $choice in
            ...
            [dD]) uninstall_script;;
            [qQ]) 
                # Снимаем ловушку перед выходом, чтобы всё было чисто
                trap - SIGINT
