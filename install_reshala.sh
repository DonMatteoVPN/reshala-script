...
# ============================================================ #
# ==      ИНСТРУМЕНТ «РЕШАЛА» v1.993 - next   ==
# ============================================================ #
# ==    1. Исправлены критические уязвимости безопасности.   ==
# ==    2. Улучшена обработка ошибок и валидация ввода.      ==
# ==    3. Повышена читаемость и надежность.                 ==
# ============================================================ #
...
readonly SCRIPT_SHA256_URL="https://raw.githubusercontent.com/DonMatteoVPN/reshala-script/refs/heads/dev/install_reshala.sh.sha256" # Новый URL для хэша
...
save_path() {
    ...
    # Валидация ключа: только буквы, цифры и подчеркивание
    if [[ ! "$key" =~ ^[a-zA-Z_][a-zA-Z0-9_]*$ ]]; then
        log "ERROR: save_path: Неверный формат ключа: $key"
        return 1
    fi
    touch "$CONFIG_FILE"
    # Используем printf вместо eval для безопасности
...
load_path() {
    ...
    # Валидация ключа: только буквы, цифры и подчеркивание
    if [[ ! "$key" =~ ^[a-zA-Z_][a-zA-Z0-9_]*$ ]]; then
        log "ERROR: load_path: Неверный формат ключа: $key"
        return 1
    fi
...
install_script() {
    if [[ $EUID -ne 0 ]]; then
        printf "%b\n" "${C_RED}❌ Эту команду — только с 'sudo'.${C_RESET}"
        exit 1
    fi
    printf "%b\n" "${C_CYAN}🚀 Интегрирую Решалу ${VERSION} в систему...${C_RESET}"
    local TEMP_SCRIPT; TEMP_SCRIPT=$(mktemp)
    local TEMP_SHA; TEMP_SHA=$(mktemp)

    # --- НОВОЕ: Скачивание с проверкой сертификатов ---
    if ! wget --ca-certificate=/etc/ssl/certs/ca-certificates.crt -q -O "$TEMP_SCRIPT" "$SCRIPT_URL"; then
        printf "%b\n" "${C_RED}❌ Не могу скачать последнюю версию. Проверь интернет или ссылку.${C_RESET}"
        log "ERROR: Не удалось скачать скрипт по URL: $SCRIPT_URL"
        rm -f "$TEMP_SCRIPT" "$TEMP_SHA"
        exit 1
    fi

    if ! wget --ca-certificate=/etc/ssl/certs/ca-certificates.crt -q -O "$TEMP_SHA" "$SCRIPT_SHA256_URL"; then
        printf "%b\n" "${C_RED}❌ Не могу скачать контрольную сумму. Проверь интернет или ссылку.${C_RESET}"
        log "ERROR: Не удалось скачать SHA256 по URL: $SCRIPT_SHA256_URL"
        rm -f "$TEMP_SCRIPT" "$TEMP_SHA"
        exit 1
    fi

    # --- НОВОЕ: Проверка контрольной суммы ---
    local computed_sha; computed_sha=$(sha256sum "$TEMP_SCRIPT" | cut -d' ' -f1)
    local expected_sha; expected_sha=$(cat "$TEMP_SHA" | cut -d' ' -f1)

    if [ "$computed_sha" != "$expected_sha" ]; then
        printf "%b\n" "${C_RED}❌ Контрольная сумма не совпадает! Файл может быть поврежден или подделан.${C_RESET}"
        log "ERROR: SHA256 не совпадает. Скачано: $computed_sha, Ожидается: $expected_sha"
        rm -f "$TEMP_SCRIPT" "$TEMP_SHA"
        exit 1
    fi
    ...
    if ! bash -n "$TEMP_SCRIPT" 2>/dev/null; then
        printf "%b\n" "${C_RED}❌ Скачанный файл содержит синтаксические ошибки.${C_RESET}"
        log "ERROR: Синтаксическая ошибка в скачанном файле."
        rm -f "$TEMP_SCRIPT"
        exit 1
    fi

    run_cmd cp -- "$TEMP_SCRIPT" "$INSTALL_PATH" && run_cmd chmod +x "$INSTALL_PATH"
    rm "$TEMP_SCRIPT"
    ...
    if [[ "${1:-}" != "update" ]]; then
        printf "   %s\n" "Установочный файл ('$0') можешь сносить."
    fi
}
...
check_for_updates() {
    ...
    local sha_url_with_buster="${SCRIPT_SHA256_URL}?cache_buster=$(date +%s)$(shuf -i 1000-9999 -n 1)"

    while [ $attempt -le $max_attempts ]; do
        # --- НОВОЕ: Используем curl с проверкой сертификатов ---
        response_body=$(curl --no-progress-meter --fail --connect-timeout 7 --max-time 15 \
            --retry 2 --retry-delay 3 --cacert /etc/ssl/certs/ca-certificates.crt \
            -s -4 -L "$url_with_buster" 2> >(sed 's/^/curl-error: /' >> "$LOGFILE"))
        curl_exit_code=$?
        ...
        if [ $curl_exit_code -eq 0 ] && [ -n "$response_body" ]; then
            ...
            if [ -n "$LATEST_VERSION" ]; then
                ...
                log "Ошибка проверки обновлений: не найдена версия."
            fi
        else
            log "Попытка $attempt: Ошибка curl при проверке обновлений. Код: $curl_exit_code"
...
run_update() {
    ...
    if [[ "$confirm_update" != "y" && "$confirm_update" != "Y" ]]; then
        printf "%b\n" "${C_YELLOW}🤷‍♂️ Ну и сиди со старьём. Твоё дело.${C_RESET}"
        wait_for_enter
        return
    fi
    ...
    # --- НОВОЕ: Скачивание с проверкой сертификатов ---
    if ! wget --ca-certificate=/etc/ssl/certs/ca-certificates.crt -4 --timeout=20 --tries=3 --retry-connrefused -q -O "$TEMP_SCRIPT" "$url_with_buster"; then
        printf "%b\n" "${C_RED}❌ Хуйня какая-то. Не могу скачать обнову. Проверь инет и лог.${C_RESET}"
        log "ERROR: Не удалось скачать обновление по URL: $url_with_buster"
        rm -f "$TEMP_SCRIPT" "$TEMP_SHA"
        wait_for_enter
        return
    fi

    if ! wget --ca-certificate=/etc/ssl/certs/ca-certificates.crt -q -O "$TEMP_SHA" "$sha_url_with_buster"; then
        printf "%b\n" "${C_RED}❌ Не могу скачать контрольную сумму обновления.${C_RESET}"
        log "ERROR: Не удалось скачать SHA256 обновления по URL: $sha_url_with_buster"
        rm -f "$TEMP_SCRIPT" "$TEMP_SHA"
        wait_for_enter
        return
    fi
    ...
    if [ "$computed_sha" != "$expected_sha" ]; then
        printf "%b\n" "${C_RED}❌ Контрольная сумма обновления не совпадает!${C_RESET}"
        log "ERROR: SHA256 обновления не совпадает. Скачано: $computed_sha, Ожидается: $expected_sha"
        rm -f "$TEMP_SCRIPT" "$TEMP_SHA"
        wait_for_enter
        return
    fi
    ...
    if [ ! -s "$TEMP_SCRIPT" ] || ! bash -n "$TEMP_SCRIPT" 2>/dev/null || [ "$downloaded_version" != "$LATEST_VERSION" ]; then
        printf "%b\n" "${C_RED}❌ Скачалось какое-то дерьмо, а не скрипт. Отбой.${C_RESET}"
        log "ERROR: Ошибка целостности или версии обновления."
        rm -f "$TEMP_SCRIPT"
        wait_for_enter
        return
    fi
...
get_docker_version() {
    ...
    local image_tag; image_tag=$(run_cmd docker inspect --format='{{.Config.Image}}' "$container_name" 2>/dev/null | cut -d':' -f2)
    if [ -n "$image_tag" ] && [ "$image_tag" != "latest" ]; then echo "$image_tag"; return; fi
    local image_id; image_id=$(run_cmd docker inspect --format='{{.Image}}' "$container_name" 2>/dev/null | cut -d':' -f2)
    echo "latest (образ: ${image_id:0:7})"
}

scan_server_state() {
    SERVER_TYPE="Чистый сервак"; PANEL_NODE_VERSION=""; PANEL_NODE_PATH=""; BOT_DETECTED=0;
    BOT_VERSION=""; BOT_PATH=""; WEB_SERVER="Не определён"
    local panel_node_container=""
    if run_cmd docker ps --format '{{.Names}}' | grep -q "^remnawave$"; then
        SERVER_TYPE="Панель"; panel_node_container="remnawave"
    elif run_cmd docker ps --format '{{.Names}}' | grep -q "^remnanode$"; then
        SERVER_TYPE="Нода"; panel_node_container="remnanode"
    fi
    if [ -n "$panel_node_container" ]; then
        PANEL_NODE_PATH=$(run_cmd docker inspect --format='{{index .Config.Labels "com.docker.compose.project.config_files"}}' "$panel_node_container" 2>/dev/null)
        PANEL_NODE_VERSION=$(get_docker_version "$panel_node_container")
    fi
    local bot_container_name="remnawave_bot"
    if run_cmd docker ps --format '{{.Names}}' | grep -q "^${bot_container_name}$"; then
        BOT_DETECTED=1
        local bot_compose_path
        bot_compose_path=$(run_cmd docker inspect --format='{{index .Config.Labels "com.docker.compose.project.config_files"}}' "$bot_container_name" 2>/dev/null || true)
    ...
    if run_cmd docker ps --format '{{.Names}}' | grep -q "remnawave-nginx"; then
        local nginx_version
    ...
    elif run_cmd docker ps --format '{{.Names}}' | grep -q "caddy"; then
        local caddy_version
...
get_hoster_info() {
    # --- НОВОЕ: Используем curl с проверкой сертификатов ---
    curl --fail --connect-timeout 5 --cacert /etc/ssl/certs/ca-certificates.crt -s ipinfo.io/org || echo "Не определён"
}
...
apply_bbr() {
    ...
    if [[ "$cake_available" == "true" ]]; then preferred_qdisc="cake"
    else
    ...
    # --- УЛУЧШЕНО: Используем printf для безопасности ---
    printf '%s\n' \
    ...
    if ! run_cmd sysctl -p "$CONFIG_SYSCTL" >/dev/null; then
        printf "%b\n" "${C_RED}❌ Ошибка применения настроек. Проверь лог.${C_RESET}"
        log "ERROR: Не удалось применить настройки из $CONFIG_SYSCTL"
        return 1
    fi
    echo ""
...
disable_ipv6() {
    ...
    printf '%s\n' \
        ...
        "net.ipv6.conf.default.disable_ipv6 = 1" | run_cmd tee /etc/sysctl.d/98-reshala-disable-ipv6.conf > /dev/null
    if ! run_cmd sysctl -p /etc/sysctl.d/98-reshala-disable-ipv6.conf > /dev/null; then
        printf "%b\n" "${C_RED}❌ Ошибка отключения IPv6. Проверь лог.${C_RESET}"
        log "ERROR: Не удалось отключить IPv6."
        return 1
    fi
...
enable_ipv6() {
    ...
    printf '%s\n' \
        ...
        "net.ipv6.conf.default.disable_ipv6 = 0" | run_cmd tee /etc/sysctl.d/98-reshala-enable-ipv6.conf > /dev/null
    if ! run_cmd sysctl -p /etc/sysctl.d/98-reshala-enable-ipv6.conf > /dev/null; then
        printf "%b\n" "${C_RED}❌ Ошибка включения IPv6. Проверь лог.${C_RESET}"
        log "ERROR: Не удалось включить IPv6."
...
view_docker_logs() {
    ...
    local original_int_handler=$(trap -p INT)
    trap "printf '\n%b\n' '${C_GREEN}✅ Возвращаю в меню...${C_RESET}'; sleep 1;" INT
    # --- УЛУЧШЕНО: Проверка возврата cd ---
    if ! cd "$(dirname "$service_path")"; then
        printf "%b\n" "${C_RED}❌ Не удалось перейти в директорию: $(dirname "$service_path")${C_RESET}"
        log "ERROR: Не удалось сменить директорию на $(dirname "$service_path")"
        if [ -n "$original_int_handler" ]; then eval "$original_int_handler"; else trap - INT; fi
        return 1
    fi
    (run_cmd docker compose logs -f) || true
    if [ -n "$original_int_handler" ]; then eval "$original_int_handler"; else trap - INT; fi
    return 0
}
...
_create_servers_file_template() {
    ...
    printf '%s\n' \
        ...
        '# root@localhost' \
        '#' \
        '# --- ДОБАВЬ СВОИ СЕРВЕРЫ НИЖЕ ---' > "$file_path"
}
...
_add_key_locally() {
    ...
    if grep -q -F "$pubkey" "$auth_keys_file"; then
        ...
        # --- УЛУЧШЕНО: Проверка формата ключа ---
        if [[ ! "$pubkey" =~ ^ssh-(rsa|dss|ed25519|ecdsa) ]]; then
            printf "    %b\n" "${C_RED}❌ Неверный формат SSH-ключа.${C_RESET}"
            log "ERROR: Неверный формат SSH-ключа при локальном добавлении: $pubkey"
            return 1
        fi
...
_ssh_add_keys() {
    ...
    # --- УЛУЧШЕНО: Проверка формата ключа ---
    if [[ ! "$PUBKEY" =~ ^ssh-(rsa|dss|ed25519|ecdsa)\ [a-zA-Z0-9+\/=]+$ ]]; then
        printf "\n%b\n" "${C_RED}❌ Это не похоже на SSH-ключ. Давай по новой.${C_RESET}"
        log "ERROR: Неверный формат SSH-ключа ввода: $PUBKEY"
        return 1
    fi
    ...
    if ! grep -q -E '[^[:space:]]' "$SERVERS_FILE_PATH" || ! grep -v -E '^\s*#|^\s*$' "$SERVERS_FILE_PATH" | read -r; then
        printf "\n%b\n" "${C_RED}❌ Файл со списком серверов пуст или содержит только комментарии. Операция прервана.${C_RESET}"
        return
    fi
    ...
    # --- УЛУЧШЕНО: Проверка формата строк в файле ---
    while IFS= read -r line || [ -n "$line" ]; do # Обработка последней строки без \n
        # Пропуск комментариев и пустых строк
        [[ -z "$line" ]] || [[ "$line" =~ ^[[:space:]]*# ]] && continue

        # Разбор строки: первая часть - хост:порт, остальное - пароль
        local host_port_part=$(echo "$line" | awk '{print $1}')
        local password=$(echo "$line" | sed "s/^$host_port_part[[:space:]]*//" | xargs) # Убираем хост и лишние пробелы
        ...
        # --- УЛУЧШЕНО: Проверка формата хоста ---
        if [[ ! "$host" =~ ^[a-zA-Z0-9._-]+@ ]]; then
            printf "\n%b\n" "${C_RED}⚠️ Неверный формат хоста в строке: $line. Пропускаю.${C_RESET}"
            log "WARNING: Неверный формат хоста: $line"
            continue
        fi
...
system_update_wizard() {
    ...
    if [[ "$confirm_upd" == "y" || "$confirm_upd" == "Y" ]]; then
        ...
        # --- УЛУЧШЕНО: Проверка возврата команд ---
        if ! run_cmd apt update; then
            printf "%b\n" "${C_RED}❌ Ошибка при apt update. Проверь лог.${C_RESET}"
            log "ERROR: apt update завершилась с ошибкой."
            return 1
        fi
        if ! run_cmd apt upgrade -y; then
            printf "%b\n" "${C_RED}❌ Ошибка при apt upgrade. Проверь лог.${C_RESET}"
            log "ERROR: apt upgrade завершилась с ошибкой."
            return 1
        fi
        if ! run_cmd apt full-upgrade -y; then
            printf "%b\n" "${C_RED}❌ Ошибка при apt full-upgrade. Проверь лог.${C_RESET}"
            log "ERROR: apt full-upgrade завершилась с ошибкой."
            return 1
        fi
        if ! run_cmd apt autoremove -y; then
            printf "%b\n" "${C_RED}❌ Ошибка при apt autoremove. Проверь лог.${C_RESET}"
            log "ERROR: apt autoremove завершилась с ошибкой."
            return 1
        fi
        if ! run_cmd apt autoclean; then
            printf "%b\n" "${C_RED}❌ Ошибка при apt autoclean. Проверь лог.${C_RESET}"
            log "ERROR: apt autoclean завершилась с ошибкой."
            return 1
        fi
        if ! run_cmd apt install -y sudo; then
            printf "%b\n" "${C_RED}❌ Ошибка при apt install sudo. Проверь лог.${C_RESET}"
            log "ERROR: apt install sudo завершилась с ошибкой."
            return 1
        fi
    ...
    if [[ "${1:-}" == "install" ]]; then
        ...
        if [[ $EUID -ne 0 ]]; then
            if [ "$0" != "$INSTALL_PATH" ]; then
                printf "%b\n" "${C_RED}❌ Запускать с 'sudo'.${C_RESET} Используй: ${C_YELLOW}sudo ./$0 install${C_RESET}"
            else
                printf "%b\n" "${C_RED}❌ Только для рута. Используй: ${C_YELLOW}sudo reshala${C_RESET}"
            fi
            exit 1;
        fi
