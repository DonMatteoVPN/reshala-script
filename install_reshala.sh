...
# ============================================================ #
# ==      ИНСТРУМЕНТ «РЕШАЛА» v0.7 - FULL VERSION      ==
# ============================================================ #
# ==    Добавлен интерактивный модуль управления SSH-ключами. ==
# ============================================================ #
...
# --- НОВЫЙ МОДУЛЬ: УПРАВЛЕНИЕ SSH-КЛЮЧАМИ ---
ssh_key_manager() {
    clear
    echo -e "${C_CYAN}--- УПРАВЛЕНИЕ SSH-КЛЮЧАМИ ---${C_RESET}"
    echo -e "Этот модуль поможет тебе массово добавить твой SSH-ключ на все твои серверы."

    # --- ШАГ 1: ИНСТРУКЦИЯ ---
    echo -e "\n${C_BOLD}[ ШАГ 1: Подготовь публичный ключ ]${C_RESET}"
    echo "На ${C_YELLOW}ТВОЁМ ЛИЧНОМ КОМПЬЮТЕРЕ${C_RESET} (не на этом сервере), открой терминал и выполни:"
    echo -e "  1. Если ключа нет, создай его:"
    echo -e "     ${C_GREEN}ssh-keygen -t ed25519 -C \"your_email@example.com\"${C_RESET}"
    echo -e "  2. Посмотри и скопируй свой ${C_YELLOW}ПУБЛИЧНЫЙ${C_RESET} ключ:"
    echo -e "     ${C_GREEN}cat ~/.ssh/id_ed25519.pub${C_RESET}"
    echo "     Скопируй всю строку, которая начинается с 'ssh-ed25519...'"

    read -p $'\nТы скопировал свой ПУБЛИЧНЫЙ ключ и готов продолжить? (y/n): ' confirm_key
    if [[ "$confirm_key" != "y" && "$confirm_key" != "Y" ]]; then
        echo -e "${C_RED}Отмена. Возвращаю в главное меню.${C_RESET}"; sleep 2; return
    fi

    # --- ШАГ 2: ПОЛУЧЕНИЕ КЛЮЧА ---
    echo -e "\n${C_BOLD}[ ШАГ 2: Вставь свой ключ ]${C_RESET}"
    read -p "Вставь сюда свой публичный ключ (ssh-ed25519...): " PUBKEY
    if ! [[ "$PUBKEY" =~ ^ssh-(rsa|dss|ed25519|ecdsa) ]]; then
        echo -e "${C_RED}❌ Это не похоже на SSH-ключ. Давай по новой.${C_RESET}"; wait_for_enter; return
    fi

    # --- ШАГ 3: СПИСОК СЕРВЕРОВ ---
    local SERVERS_FILE="servers.txt"
    echo -e "\n${C_BOLD}[ ШАГ 3: Подготовь список серверов ]${C_RESET}"
    echo "Теперь на этом сервере, в текущей директории, создай файл с именем ${C_YELLOW}${SERVERS_FILE}${C_RESET}"
    echo "Внутри файла укажи каждый сервер на новой строке в формате ${C_GREEN}пользователь@адрес${C_RESET}."
    echo "Пример:"
    echo "  root@1.2.3.4"
    echo "  admin@mydomain.com"
    
    read -p $'\nТы создал файл servers.txt и готов к установке? (y/n): ' confirm_servers
    if [[ "$confirm_servers" != "y" && "$confirm_servers" != "Y" ]]; then
        echo -e "${C_RED}Отмена. Возвращаю в главное меню.${C_RESET}"; sleep 2; return
    fi

    if [ ! -f "$SERVERS_FILE" ]; then
        echo -e "${C_RED}❌ Файл '${SERVERS_FILE}' не найден. Убедись, что он в той же папке, откуда запущен Решала.${C_RESET}"; wait_for_enter; return
    fi

    # --- ШАГ 4: УСТАНОВКА ---
    echo -e "\n${C_BOLD}[ ШАГ 4: Установка ключа на серверы ]${C_RESET}"
    echo "Сейчас я буду по очереди подключаться к каждому серверу."
    echo -e "${C_YELLOW}Тебе нужно будет ввести пароль для каждого сервера.${C_RESET}"
    wait_for_enter

    if command -v ssh-copy-id &> /dev/null; then
        echo "Использую 'ssh-copy-id' (безопасный метод)..."
        local TEMP_KEY_FILE; TEMP_KEY_FILE=$(mktemp)
        echo "$PUBKEY" > "$TEMP_KEY_FILE"
        ...
        while IFS= read -r host || [[ -n "$host" ]]; do
            ...
            if ssh-copy-id -i "$TEMP_KEY_FILE" -o ConnectTimeout=10 -o StrictHostKeyChecking=no "$host"; then
                echo -e "    ${C_GREEN}✅ Успех!${C_RESET}"
            else
                echo -e "    ${C_RED}❌ Ошибка. Проверь пароль или доступность хоста.${C_RESET}"
            fi
        done < "$SERVERS_FILE"
        rm "$TEMP_KEY_FILE"
    else
        echo "ВНИМАНИЕ: 'ssh-copy-id' не найден. Использую ручной метод (менее надёжный)."
        while IFS= read -r host || [[ -n "$host" ]]; do
            if [[ -z "$host" ]] || [[ "$host" =~ ^# ]]; then continue; fi
            echo -e "\n${C_CYAN}--> Добавляю ключ на $host...${C_RESET}"
            local ssh_command="mkdir -p ~/.ssh && touch ~/.ssh/authorized_keys && grep -q -F '$PUBKEY' ~/.ssh/authorized_keys || echo '$PUBKEY' >> ~/.ssh/authorized_keys && chmod 700 ~/.ssh && chmod 600 ~/.ssh/authorized_keys"
            if ssh -o ConnectTimeout=10 -o StrictHostKeyChecking=no "$host" "$ssh_command"; then
                echo -e "    ${C_GREEN}✅ Успех!${C_RESET}"
            else
                echo -e "    ${C_RED}❌ Ошибка. Проверь пароль или доступность хоста.${C_RESET}"
            fi
...
show_menu() {
    while true; do
        ...
        case $choice in
            ...
            7) ssh_key_manager; wait_for_enter;;
            [uU]) if [[ ${UPDATE_AVAILABLE:-0} -eq 1 ]]; then run_update; else echo "Ты слепой?"; sleep 2; fi;; [dD]) uninstall_script;; [qQ]) echo "Был рад помочь. Не обосрись. 🥃"; break;; *) echo "Ты прикалываешься?"; sleep 2;;
        esac
    done
}
