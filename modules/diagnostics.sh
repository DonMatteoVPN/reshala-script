...
_show_docker_cleanup_menu() {
    while true; do
        ...
        echo "----------------------------------------"; echo "   1. 📊 Показать самые большие образы"; echo "   2. 🧹 Простая очистка (висячие образы, кэш)"; echo "   3. 💥 Полная очистка неиспользуемых образов"; echo "   4. 🗑️ Очистка неиспользуемых томов (ОСТОРОЖНО!)"; echo "   5. 📈 Показать итоговое использование диска"; echo "   b. Назад"
        echo "----------------------------------------"; local choice; read -r -p "Твой выбор: " choice
        case "$choice" in
...
show_diagnostics_menu() {
    while true; do
        ...
        printf "%b\n" "${C_CYAN}╚══════════════════════════════════════════════════════════════╝${C_RESET}"
        echo ""; echo "   [1] 📜 Посмотреть журнал «Решалы»"; echo "   [2] 🐳 Управление Docker (Очистка)"; echo ""; echo "   [b] 🔙 Назад в главное меню"; echo "------------------------------------------------------"
