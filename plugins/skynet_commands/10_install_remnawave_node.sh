...
#   NODE_SECRET_KEY   — приватный x25519 ключ от панели (опционально, иначе плейсхолдер)
#   NODE_PORT         — порт ноды (опционально, по умолчанию 2222)
#   CERT_MODE         — пока не используется (TLS для удалённых нод будет добавлен позже)
#
set -euo pipefail
